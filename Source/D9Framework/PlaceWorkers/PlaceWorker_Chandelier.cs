...
{
    public class PlaceWorker_Chandelier : PlaceWorker
    {
		public override AcceptanceReport AllowsPlacing(BuildableDef checkingDef, IntVec3 loc, Rot4 rot, Map map, Thing thingToIgnore = null) {
            ...
            foreach (IntVec3 c in GenAdj.CellsOccupiedBy(loc, rot, checkingDef.Size))
            {
                if (c.GetEdifice(map) != null)
                {
                    ...
                    {
                        return new AcceptanceReport("AJO_ObjectTooTall".Translate(c.GetEdifice(map).LabelCap, checkingDef.LabelCap));
                    }
                }
                if (c.GetThingList(map).Find(ch => ch is Building) != null)
                {
